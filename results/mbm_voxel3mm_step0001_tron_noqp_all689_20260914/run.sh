...
#
# Tron-style no-QP CBF gate over all 689 VAMP problems: nominal controls are clipped to
# the speed box, then admitted unchanged or rejected; no scalar braking and no QP.
# endpoint eligibility screening bypassed and all runtime CBF margins/buffers zero.
# The QP work counters are plumbed through to
# the CSV: qp_calls, qp_rows, qp_solves (ControlFilter::Counters). qp_rows/qp_calls is
# the constraint-row count of the program the filter assembled per call, which is what
# `screening` and `activePairs` are supposed to move -- qpPlain has both off and so sits
# at the full 343 rows (40 spheres + 303 self pairs) with a solve every call. isSafe and
# qpFreeGate build no QP and report zero. Identical configuration otherwise, same seed,
# so every other column reproduces the earlier run.
# Screening on, RRT-Connect range 2.0 rad, world/self margins and buffers all 0.
#
...
ARGS=("$SCENES"      100  10.0 0.003 0.001   2.0    0.0    0.0      -1    20      -1        0.0     ""       -1    1 )
...
head -1 "$OUT/shard0.csv" >"$OUT/voxel3mm_step0001_tron_noqp_all689.csv"
for i in $(seq 0 $((SHARDS - 1))); do
    tail -n +2 "$OUT/shard$i.csv" >>"$OUT/voxel3mm_step0001_tron_noqp_all689.csv"
done
echo "rows: $(tail -n +2 "$OUT/voxel3mm_step0001_tron_noqp_all689.csv" | wc -l)"
