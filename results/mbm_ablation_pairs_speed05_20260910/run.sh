...
OUT=results/mbm_ablation_pairs_speed05_20260910
...
# 0.5 rad/s, not 10: at 10 the screening threshold is ~0.85 m in a ~1 m workspace, so
# nothing is ever screened out and neither screening nor activePairs can prune anything
# -- measured inert (5.36 vs 5.07 ms) there, and worth 18-22% here.
export OMPL_CBF_MAX_SPEED=0.5
...
# qp-free repairs by scaling the control instead of rejecting it outright. Default
# now, but stated explicitly here because it changes what that row measures: 0 gives
# the published LQR-CBF-RRT* accept-or-reject decision. See UR5QPFreeGate.
export OMPL_QPFREE_REPAIR=1
# Audit at 0.002 rad, under the 3 mm buffer it checks, not the 0.02 rad default that
# is an order of magnitude coarser than the margin. At 0.02 rad the baseline reads
# 1-2 unsafe at -0.02 mm; at 0.002 rad the same paths read 24 unsafe at -0.32 mm, so
# the coarse column was measuring the audit, not the planner. Costs ~1.4% wall time.
export OMPL_MBM_AUDIT_RES=0.002
