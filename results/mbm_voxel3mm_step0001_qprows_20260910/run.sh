...
#
# Rerun of mbm_voxel3mm_step0001_20260910 with the QP work counters plumbed through to
...
OUT=results/mbm_voxel3mm_step0001_qprows_20260910
...
head -1 "$OUT/shard0.csv" >"$OUT/voxel3mm_step0001_qprows.csv"
for i in $(seq 0 $((SHARDS - 1))); do
    tail -n +2 "$OUT/shard$i.csv" >>"$OUT/voxel3mm_step0001_qprows.csv"
done
echo "rows: $(tail -n +2 "$OUT/voxel3mm_step0001_qprows.csv" | wc -l)"
