...
configs = [('escape_off',0,8,1,1,'0.0'), ('escape_on',0,8,1,1,'0.25')]
scenes  = ['empty','pillars','clutter','shelf','corridor']
rows=[]
for scene in scenes:
    for seed in range(3):
        for name,iters,window,workers,adapt,esc in configs[seed%2:]+configs[:seed%2]:
            cmd=[demo,str(out/(scene+'.problem')),'2','','3','-1','-1','-1','0','0','-1',
                 '1.5','0.10','-1',str(seed*100),str(iters),str(window),str(workers),
                 '1','1','40','3',str(adapt),'0',esc]
...
with open('/tmp/claude-1000/-home-ax-ml-ubu24-ompl/63204d42-c153-4e02-a96c-a20322937cac/scratchpad/esc_ab.csv','w') as f:
