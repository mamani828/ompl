...
#
# No-QP CBF gate with envelope-certified adaptive hops over all 689 VAMP problems, with
...
head -1 "$OUT/shard0.csv" >"$OUT/voxel3mm_step0001_qpfree_envelope_all689.csv"
for i in $(seq 0 $((SHARDS - 1))); do
    tail -n +2 "$OUT/shard$i.csv" >>"$OUT/voxel3mm_step0001_qpfree_envelope_all689.csv"
done
echo "rows: $(tail -n +2 "$OUT/voxel3mm_step0001_qpfree_envelope_all689.csv" | wc -l)"
