#!/usr/bin/env python3
"""Generate a dependency-free C++ kinematics/sphere table from a spherized URDF.

Usage:
  python3 scripts/generate_robot_model.py \
      /path/to/<robot>_spherized.urdf /path/to/<robot>.srdf \
      src/ompl/robots/<Robot>.h --class-name <Robot> --joints a,b,c

This is `generate_reachy2_model.py` with the three robot-specific facts lifted
out: which joints are active, what the class is called, and where the base sits.

**The joint order is the argument, not the URDF order, and it matters.** A
planner's configurations come from somewhere -- for the MotionBenchMaker sets
that is VAMP -- and a model whose joints are ordered differently from the source
of its configurations produces kinematics that are wrong without being obviously
wrong. Baxter is the live example: the URDF declares the right arm first and
VAMP orders the left arm first. Pass `--joints` in the order the configurations
will arrive in, and check the result against a reference FK.

Joints not listed are fixed at their URDF zero pose, which is what keeps
gripper mimic joints closed.
"""
...
def main():
    ...
    ap.add_argument("--class-name", default="Reachy2")
    ap.add_argument("--joints", default=None,
                    help="comma-separated active joints, in the order configurations arrive in; "
                         "default is Reachy2's arm list")
    ap.add_argument("--base-z", type=float, default=None,
                    help="height of the URDF root above the contact plane (default 0.1075, Reachy2's)")
    ap.add_argument("--tips", default=None,
                    help="comma-separated left,right tip links; omitted if the robot has none")
    args = ap.parse_args()

    global ACTIVE
    ACTIVE = tuple(args.joints.split(",")) if args.joints else REACHY2_ACTIVE
    class_name = args.class_name
    base_z = 0.1075 if args.base_z is None else args.base_z
    tips = args.tips.split(",") if args.tips else (
        ["l_arm_tip_bottom", "r_arm_tip_bottom"] if args.joints is None else None)
    ...
    lines = [
        ...
        "namespace ompl::robots {", f"class {class_name} {{", "public:",
        ...
        f"  explicit {class_name}(const Eigen::Isometry3d &basePose = groundedBasePose()) : basePose_(basePose) {{}}",
        f"  static Eigen::Isometry3d groundedBasePose() {{ Eigen::Isometry3d pose=Eigen::Isometry3d::Identity(); pose.translation().z()={fmt(base_z)}; return pose; }}",
    ...
    if tips is not None:
        lines += [
            f"  static constexpr int leftTipLink = {link_index[tips[0]]};",
            f"  static constexpr int rightTipLink = {link_index[tips[1]]};",
        ]
    lines += [
    ...
    if tips is not None:
        tipcode = [
            ...
            "  static Jacobian tipJacobian(const Kinematics&k,bool left){ int li=left?leftTipLink:rightTipLink; auto p=k.link[li].translation(); return pointJacobian(k,p,left?"
            + str(masks[link_index[tips[0]]]) + ":" + str(masks[link_index[tips[1]]]) + "); }",
        ]
        cut = lines.index("private:") if "private:" in lines else lines.index("  static Eigen::Vector3d sphereCenter(const Kinematics&k,std::size_t i){ const auto&s=spheres()[i]; return k.link[s.link]*Eigen::Vector3d(s.center.data()); }")
        lines[cut:cut] = tipcode
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text("\n".join(lines))
    print(f"wrote {args.output}: {class_name}, {len(ACTIVE)} joints, {len(spheres)} spheres, {len(pairs)} self pairs")
